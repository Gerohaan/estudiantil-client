  ...
  <q-page>
    <q-dialog v-model="modalAddValue" full-width persistent>
        ...
        <q-bar class="bg-indigo-1">
          <span class="text-center">{{ Object.entries(rowUpdate).length === 0 ? 'Registro de sección' : 'Edición de sección'  }} </span>
  ...
  methods: {
    ...
    async onSubmit(){
      try {
        ...
        let dispatchDir = Object.entries(this.rowUpdate).length === 0 ? 'sections/registerS' : 'sections/editS' 
        await this.$store.dispatch(dispatchDir, section)
        let message =  Object.entries(this.rowUpdate).length === 0 ? 'Sección creada' : 'Sección actualizada' 
        Notify.create(
            { message: message, 
              type: 'positive', 
              position: 'top-right'
